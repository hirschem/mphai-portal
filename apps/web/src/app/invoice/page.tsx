...
export default function InvoicePage() {
  ...
  useEffect(() => {
    ...
    apiFetchOptional<AdminSaveInvoiceResp>(`/api/admin-saves/invoice/${entityId}`).then(
      ({ ok, status, data }) => {
        if (ok && data) {
          if (data.proposalData) setProposalData(data.proposalData);
          if (data.sessionId !== undefined) setSessionId(data.sessionId || null);
          return;
        }
  ...
  const persistAdminSave = useCallback(
    (sid: string, pdata: unknown) => {
      if (!isAdmin) return;

      void apiFetch<{ success?: boolean }>(`/api/admin-saves/invoice/${sid || "current"}`, {
        method: "PUT",
  ...
  const devSaveKey =
    process.env.NODE_ENV !== "production" && isAdmin ? `invoice/${sessionId || "current"}` : null;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <h1 className="text-4xl font-bold text-gray-900 mb-2">Invoice Mode</h1>
              <p className="text-gray-600">Transform handwritten proposals into professional invoices</p>
            </div>
        ...
        <UploadForm
          onSuccess={(sid, data) => {
            ...
            persistAdminSave(sid, data);
          }}
        />

        {Boolean(proposalData) && Boolean(sessionId) && (
          <ProposalDisplay data={proposalData} sessionId={sessionId!} />
        )}
      </div>
    </main>
  );
